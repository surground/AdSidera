...
def start(message):
    ...
    zodiac = bot.send_message(message.chat.id, "Укажите ваш знак зодиака:",
                              reply_markup=keyboard)
@bot.message_handler(commands=['criminal'])
def criminal(message):
    url = 'https://www.ugra.kp.ru/daily/kriminal/'
    response = requests.get(url)
    soup = BeautifulSoup(response.text, 'lxml')
    quotes = soup.find_all('a', class_='styled__TitleLink-sc-1tputnk-2 dwMuZb')
    for i in quotes:
        bot.send_message(message.chat.id, f"[{i.span.text}]({parse.urljoin('https://www.irk.kp.ru/', i['href'])}",
                         parse_mode='Markdown', disable_web_page_preview=True)
@bot.message_handler(content_types=['text'])
def get_text_messages(message):
    if message.text.lower() == "привет":
        send = bot.send_message(message.from_user.id, "Привет, сейчас я расскажу тебе гороскоп на сегодня.\nНапиши 'давай' или 'хорошо', чтобы начать")
        bot.register_next_step_handler(send, answer)
    elif message.text == "/help":
        bot.send_message(message.from_user.id, "Напишите /start, чтобы начать,\nНапишите /criminal, чтобы узнать криминальные сводки")
    else:
        bot.send_message(message.from_user.id, "Я тебя не понимаю. Напиши /help.")
def answer(message):
    if message.text in ['давай', 'хорошо']:
        start(message)
